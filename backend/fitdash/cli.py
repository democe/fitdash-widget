...
from fitdash.model import present, today
...
def execute(args):
    ...
    if args.command == "demo":
        ...
        elif auth.get("status") not in (None, "connected", "authorizing"):
        ...
        with cache.lock():
            if args.command == "auth":
                ...
                creds = vault.load()
                vault.clear()
                for name in ("snapshot.json", "scheduler.json", "auth.json"):
                    cache.remove(name)
...
def main():
    ...
    try:
        ...
        data = snapshot(Cache(args.state_dir))
        data["status"] = str(error)
        data = present(
            data,
            time.time(),
            "metric" if args.units == "auto" else args.units,
            today(args.timezone or data.get("timezone", "")).isoformat(),
            args.interval,
        )
        # Persist a safe error state so a detached auth/sync is visible to the UI.
        Cache(args.state_dir).write("snapshot.json", data)
    ...
    if args.command == "auth" and data.get("status") == "backend_error":
        Cache(args.state_dir).write("auth.json", {"status": "backend_error"})
